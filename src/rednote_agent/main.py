...
# Task
你的任务是根据用户提供的产品和需求，生成包含标题 title、正文 body、相关标签 hashtags 和表情符号 emojis 的完整小红书笔记。

# Rules
- 请始终采用 Thought-Action-Observation 模式进行推理和行动
- 文案风格需活泼、真诚、富有感染力，在生成文案前，请务必先思考并收集足够的信息
- 当完成任务后，请直接输出 strict raw JSON 格式，禁止输出 markdown 或代码块，格式如下：
...
def generate_rednote(product_name: str, tone_style: str = "活泼甜美", max_iterations: int = 5) -> str:
    ...
    messages = [
        ...
        {"role": "user", "content": f"请为产品「{product_name}」生成一篇小红书爆款文案。要求：语气{tone_style}，包含标题、正文、至少5个相关标签和5个表情符号。"}
    ]
    ...
    while iteration_count < max_iterations:
        ...
        try:
            ...
            response = client.chat.completions.create(
                ...
                response_format={ "type": "json_object" } # 指定模型必须输出的格式
            )
